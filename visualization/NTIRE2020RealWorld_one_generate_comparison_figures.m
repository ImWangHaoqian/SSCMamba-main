%% Generate Comparison Figures for ARAD_HS_0463 (NTIRE2020_realworld)
...
output_base_dir = fullfile(script_dir, 'comparison_results', 'NTIRE2020_realworld_ARAD_HS_0463');
...
% Target channels for NTIRE2020_realworld (31 channels, 400-700nm, 10nm steps)
...
data_base_path = fullfile(script_dir, '..', '..', '..', 'exp', 'NTIRE2020_realworld');
data_file_name = 'ARAD_HS_0463.mat';
...
% Contrast enhancement parameters for difference heatmap
diff_percentile_low = 2;   % 使用2%百分位数作为最小值（去除极值）
diff_percentile_high = 98;  % 使用98%百分位数作为最大值（去除极值）
diff_gamma = 0.5;           % Gamma校正值（<1增强对比度，>1降低对比度）
...
% For NTIRE2020_realworld (31 channels, 400-700nm, 10nm steps):
% - Channel 6 (450nm) - Blue region
% - Channel 16 (550nm) - Green region  
% - Channel 26 (650nm) - Red region
if num_channels >= 26
    rgb_channels = [26, 16, 6];  % [R, G, B] channel indices for NTIRE2020_realworld
else
...
for method_idx = 1:length(methods)
    ...
    % Load data file
    % GT is in the same NTIRE2020_realworld directory, just in a subfolder named 'GT'
        ...
        if ~strcmp(method_name, 'GT') && ~isempty(gt_cropped_data)
            try
                ...
                if ~isequal(size(cropped_data), size(gt_cropped_data))
                    ...
                    % Enhanced normalization with percentile clipping and gamma correction
                    % Use percentiles to remove outliers and enhance contrast
                    diff_min = prctile(diff_mean(:), diff_percentile_low);
                    diff_max = prctile(diff_mean(:), diff_percentile_high);
                    if diff_max > diff_min
                        % Clip values outside percentile range
                        diff_mean_clipped = max(diff_min, min(diff_max, diff_mean));
                        % Normalize to [0, 1]
                        diff_normalized = (diff_mean_clipped - diff_min) / (diff_max - diff_min);
                        % Apply gamma correction to enhance contrast
                        diff_normalized = diff_normalized .^ diff_gamma;
                    else
                        diff_normalized = zeros(size(diff_mean));
                    end
                    fprintf('    Difference range: min=%.6f, max=%.6f (after percentile clipping)\n', diff_min, diff_max);
        ...
        try
            ...
            try
                ...
                try
                    ...
                    fprintf('  2. Check if other files in the same directory can be read (e.g., ARAD_HS_0463.mat)\n');
