%% Generate Comparison Figures for ARAD_1K_0924
% This script generates cropped region visualizations for 13 methods
% across 3 channels (22-Red, 11-Green, 1-Purple)
...
% Target channels: 22 (Red), 11 (Green), 1 (Purple)
target_channels = [27, 17, 1];
...
% Based on show_real.m, NTIRE2022 uses 28 channels
...
% For NTIRE2022 (28 channels, 453.5-648.0nm):
% - Channel 1 (453.5nm) - Blue region
% - Channel 11 (around 510nm) - Green region  
% - Channel 22 (around 625nm) - Red region
if num_channels >= 22
    rgb_channels = [27, 17, 1];  % [R, G, B] channel indices
else
...
for method_idx = 1:length(methods)
    ...
    % Load data file
    % GT is in the same NTIRE2022 directory, just in a subfolder named 'GT'
    ...
    try
        ...
        for ch_idx = 1:length(target_channels)
            ...
            if channel <= length(lam28)
                wavelength = lam28(channel);
            else
                % Fallback: estimate wavelength
                wavelength = 450 + (channel - 1) * 10;
            end
        ...
        try
            ...
            try
                ...
                try
                    ...
                    fprintf('  2. Check if other files in the same directory can be read (e.g., ARAD_1K_0901.mat)\n');
