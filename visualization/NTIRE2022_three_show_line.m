%% Plot Spectral Curves Comparison for ARAD_1K_0924
% This script generates spectral curves comparison for 14 methods (including GT)
% Similar to show_line.m but adapted for NTIRE2022 dataset
...
data_base_path = fullfile(script_dir, '..', '..', '..', 'exp', 'NTIRE2022');
data_file_name = 'ARAD_1K_0924.mat';

% Wavelength configuration for NTIRE2022 (28 channels)
...
output_base_dir = fullfile(script_dir, 'comparison_results', 'ARAD_1K_0924');
...
if num_channels >= 22
    rgb_channels = [22, 11, 1];  % [R, G, B] channel indices
else
...
% If data has 31 channels but lam28 has 28, we need to handle this
actual_num_channels = num_channels;
if actual_num_channels == 31 && length(lam28) == 28
    % Use first 28 channels or create extended wavelength vector
    % For now, use first 28 channels to match lam28
    fprintf('Data has %d channels, but lam28 has %d wavelengths.\n', actual_num_channels, length(lam28));
    fprintf('Using first %d channels to match wavelength vector.\n', length(lam28));
    channels_to_use = 1:length(lam28);
    wavelengths_to_use = lam28;
elseif actual_num_channels == length(lam28)
    ...
    % Estimate wavelengths: 450nm to 650nm range
    wavelengths_to_use = linspace(450, 650, actual_num_channels);
    channels_to_use = 1:actual_num_channels;
end
...
output_figure_path = fullfile(output_base_dir, 'spectral_comparison_ARAD_1K_0924.png');
